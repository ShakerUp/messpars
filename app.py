...
from telethon.tl.types import MessageActionTopicCreate, MessageMediaPhoto, MessageMediaDocument
...
class DBManager:
    ...
    def save_relation(source_msg_id, target_msg_id, topic_id):
        with sqlite3.connect(DB_FILE) as conn:
            conn.execute('INSERT OR REPLACE INTO message_map (source_id, target_msg_id, topic_id) VALUES (?, ?, ?)',
    ...
    def get_relation(source_id):
        try:
            with sqlite3.connect(DB_FILE) as conn:
                r = conn.execute('SELECT target_msg_id, topic_id FROM message_map WHERE source_id = ?', (source_id,)).fetchone()
...
class ForumManager:
    ...
    async def get_or_create_topic(chat_id, chat_title, source_thread_id, source_thread_name):
        ...
        if topic_id and (chat_id, source_thread_id) in VALID_TOPICS: return int(topic_id)
        if topic_id:
            if await ForumManager._validate_topic(topic_id, chat_id, source_thread_id): return int(topic_id)
        
        name = (f"{source_thread_name} | {chat_title}" if source_thread_name else f"💬 {chat_title}")[:120]
        result = await bot_app.bot.create_forum_topic(TARGET_CHAT_ID, name)
    ...
    async def send_to_topic(msg, chat_id, chat_title, source_thread_id, source_thread_name):
        ...
        for attempt in (1, 2):
            ...
            if not tid: return
            try:
                p = {"chat_id": TARGET_CHAT_ID, "message_thread_id": tid, "caption": text}
                if msg.media:
                    buf = io.BytesIO()
                    ...
                    sent = await bot_app.bot.send_message(chat_id=TARGET_CHAT_ID, message_thread_id=tid, text=text)
                ...
                if attempt == 1: TopicManager.remove(chat_id, source_thread_id); continue
...
async def msg_handler(event):
    ...
    if msg.action: return # Пропускаем системные сообщения
    ...
    if msg.reply_to and msg.reply_to.forum_topic:
        ...
        try:
            ...
            if m and m[0].action and hasattr(m[0].action, 'title'):
...
async def edit_handler(event):
    ...
    try:
        ...
        if msg.media:
            await bot_app.bot.edit_message_caption(TARGET_CHAT_ID, relation["target_msg_id"], caption=new_text)
        else:
            await bot_app.bot.edit_message_text(TARGET_CHAT_ID, relation["target_msg_id"], text=new_text)
    except: pass
...
async def main():
    ...
    await client.start()
    print("🚀 Система запущена. Мирроринг активен.")
